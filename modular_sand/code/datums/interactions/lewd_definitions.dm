					...
					switch(target_orifice)
						...
						if(CUM_TARGET_FEET)
							if(!last_lewd_datum.require_target_num_feet)
								...
								if(partner.has_feet())
									message = "cums on [partner_name]'s [last_lewd_datum.require_target_num_feet == 1 ? pick("foot", "sole") : pick("feet", "soles")]."
								else
									message = "cums on the floor!"
					...
					switch(target_orifice)
						...
						if(CUM_TARGET_FEET)
							if(!last_lewd_datum.require_target_num_feet)
								...
								if(partner.has_feet())
									message = "squirts on [partner_name]'s [last_lewd_datum.require_target_num_feet == 1 ? pick("foot", "sole") : pick("feet", "soles")]."
								else
									message = "squirts on the floor!"
						//weird shit goes here
						if(CUM_TARGET_EARS)
